...
from config.private.tail.config_loader import TailConfig
mcp = FastMCP("Tail MCP Server", host="0.0.0.0", port=TailConfig().get_config().private_config.port)


@mcp.tool(
    name="tail_file_view_tool"
    if TailConfig().get_config().public_config.language == LanguageEnum.ZH
    else
    "tail_file_view_tool",
    description='''
    使用tail命令快速查看文件末尾部分内容
    1. 输入值如下：
        - host: 远程主机名称或IP地址，若不提供则表示查看本机指定文件内容
        - num: 想要查看的行数，不指定默认查看后10行
        - file: 要查看的文件
    2. 返回值为字符串，为返回的文件内容
    '''
    if TailConfig().get_config().public_config.language == LanguageEnum.ZH
    ...
    '''
    Use the tail command to quickly view the end content of a file
    1. Input values are as follows:
        ...
        - num: The number of lines you want to view. If not specified, the default is to view the last 10 lines.
        - file: The file to be viewed.
    2. The return value is a string, which is the content of the returned file.
    '''

)
def tail_file_view_tool(host: Union[str, None] = None, num: int = 10, file: str = None) -> str:
    """使用tail命令快速查看文件末尾部分内容"""
    if host is None:
        try:
            command = ['tail', '-n', f'{num}', f'{file}']
        ...
        for host_config in TailConfig().get_config().public_config.remote_hosts:
