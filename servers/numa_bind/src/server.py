...
from config.private.numa_bind.config_loader import NumaBindConfig

mcp = FastMCP("NUMA Bind MCP Server", host="0.0.0.0", port=NumaBindConfig().get_config().private_config.port)

@mcp.tool(
    name="numa_bind_tool"
    if NumaBindConfig().get_config().public_config.language == LanguageEnum.ZH
    else "numa_bind_tool",
    description='''
